...
def parse_args():
    ...
    csv_group.add_argument(
        ...
        default=None,
        help="The maximum number of text ngram "
             "explanations that will be generated for "
             "each prediction. Set to 'all' to return all ngrams or an integer value for a"
             "specific max number of ngrams.",
    )
    csv_group.add_argument(
        "--explanation_num_top_classes",
        type=int,
        default=None,
        help="Number of top predicted classes for each row that will be explained. "
             "Only for multiclass and clustering explanations.",
    )
    csv_group.add_argument(
        ...
        nargs="?",
        default=None,
        help="List of class names that will be explained for each row. "
             "Only for multiclass and clustering explanations.",
    )
    csv_group.add_argument(
        "--keep_cols",
    ...
    csv_group.add_argument(
        ...
        type=str,
        default=None,
        help="Prefix for batch name that will be used for tracking predictions"
    )
    csv_group.add_argument(
        "--chunk_size",
        type=str,
        default=None,
        help="Which strategy should be used to determine the chunk size. "
...
def main():
    ...
    # Configure multiclass or clustering explanations mode
    if args.explanation_num_top_classes:
        payload["explanationNumTopClasses"] = args.explanation_num_top_classes
    if args.explanation_class_names:
        payload["explanationClassNames"] = args.explanation_class_names.split(",")
    if args.max_ngram_explanations:
        payload["maxNgramExplanations"] = args.max_ngram_explanations
    ...
    if args.batch_prefix:
        payload["monitoringBatchPrefix"] = args.batch_prefix

    if args.chunk_size:
        if args.chunk_size not in ('auto', 'fixed', 'dynamic'):
            try:
                args.chunk_size = int(args.chunk_size)
            except ValueError:
                logger.info(
                    "--chunk_size must be either 'auto', 'fixed', 'dynamic', "
                    "or positive integer"
                )
                return 1

        payload['chunkSize'] = args.chunk_size
